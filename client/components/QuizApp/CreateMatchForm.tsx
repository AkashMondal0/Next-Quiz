...
// ============= CREATE MATCH FORM =============
const CreateMatchForm = memo(function CreateMatchForm({ isDark, textPrimaryClass, textSecondaryClass, cardBgClass, formData, setFormData, onBack }: { isDark: boolean, textPrimaryClass: string, textSecondaryClass: string, cardBgClass: string, formData: any, setFormData: React.Dispatch<React.SetStateAction<any>>, onBack: () => void }) {
      ...
      <Card className={`${cardBgClass} border-2 backdrop-blur-xl shadow-xl`}>
        <CardHeader className="space-y-4">
          ...
          >
            ...
            <div className="absolute -inset-2 bg-gradient-to-r from-blue-500 to-blue-600 rounded-2xl opacity-20 blur-xl" style={{ animation: 'pulse 2.2s infinite' }} />
          </motion.div>
        ...
        <CardContent className="space-y-6">
          ...
          <div className="space-y-2">
            ...
            <Textarea
              ...
              rows={3}
              className={`text-base resize-none ${isDark ? 'bg-slate-800 border-slate-700 text-white placeholder:text-slate-500' : 'bg-white border-purple-200'
                }`}
            />
            ...
            >
              {[
                ...
                <Label
                  key={level.value}
                  htmlFor={level.value}
                  className={`flex items-center justify-center p-4 rounded-xl border-2 cursor-pointer transition-all ${formData.difficulty === level.value
          ...
          <Counter
            ...
            onChange={(val) => handleChange('participantLimit', val)}
            min={2}
            max={20}
            isDark={isDark}
            ...
            <div className="grid grid-cols-4 gap-2">
              {[5, 10, 15, 20].map((duration) => (
                <Button
                  ...
                  className={`h-12 font-bold ${formData.duration === duration
