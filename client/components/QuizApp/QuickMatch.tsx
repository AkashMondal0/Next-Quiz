...
const QuickMatch = memo(function QuickMatch({
  isDark,
  textPrimaryClass,
  textSecondaryClass,
  cardBgClass,
  ...
  const onSubmit = async (data: MatchFormData) => {
    try {
      ...
      const { aiPrompt, difficulty, participantLimit, duration, numberOfQuestions } = data;
      ...
      const res = await api.post("/quiz/quick/create", payload);
      router.push(`/quick/${res.data.code}`);
    } catch (error) {
        ...
        <CardHeader className="space-y-4">
          ...
          <div>
            ...
            <CardDescription className={`text-sm md:text-base ${textSecondaryClass}`}>
                Create a quick quiz match with AI-generated questions
            </CardDescription>
          </div>
        </CardHeader>
